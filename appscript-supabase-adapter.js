(function () {
  ...
  async function handlePost(action, body) {
    ...
    if (action === "createOrder") {
      ...
      const { data } = await insert("orders", order);
  ...
  async function adminData() {
    const [products, extras, orders, tables, payments, inventory, inventoryMovements, cashiers] = await Promise.all([
  ...
  async function menuData() {
    ...
    return { products, extras, updatedAt: now() };
  }
