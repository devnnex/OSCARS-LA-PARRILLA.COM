  ...
  el.orderDetailContent.innerHTML = `
    ...
    <div class="order-detail-grid">
      ...
      <section>
        ...
        <p>${escapeHtml(order.notas || order.note || "Sin notas")}</p>
      </section>
    </div>
    <div class="order-detail-items">
      ...
      ${items.length ? items.map(item => `
        <div class="order-detail-item">
          <div>
            <strong>${escapeHtml(item.nombre || item.name || "Producto")}</strong>
            <small>${escapeHtml(item.detalle || item.option || item.opcion || "")}</small>
          </div>
          <span>${moneyToNumber(item.qty || item.cantidad || 1)} x ${formatMoney(item.precio || item.price)}</span>
          <strong>${formatMoney((moneyToNumber(item.qty || item.cantidad || 1) || 1) * moneyToNumber(item.precio || item.price))}</strong>
        </div>
...
function renderProducts() {
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (!term) return true;
    return normalize(`${product.nombre} ${product.descripcion} ${product.categoria_id} ${labelFromId(product.categoria_id)} ${product.precio}`).includes(term);
  });
...
function orderItemsLabel(itemsValue) {
  if (Array.isArray(itemsValue)) {
    ...
    return itemsValue.map(item => `${item.cantidad || item.qty || 1}x ${item.nombre || item.title || "Producto"}`).join(", ");
  }
  if (itemsValue && typeof itemsValue === "object") {
    return `${itemsValue.cantidad || itemsValue.qty || 1}x ${itemsValue.nombre || itemsValue.title || "Producto"}`;
  }
  try {
